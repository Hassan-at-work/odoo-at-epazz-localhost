...
def _rpc_count_modules(addr='http://crm-stg.boxesos.com', port=8069, dbname='mycompany'):
    time.sleep(5)
...
class Docker():
    ...
    def run(self, cmd, build_dir, container_name, user='odoo', exposed_port=None, detach=False, timeout=None):
        ...
        if exposed_port:
            docker_cmd.extend(['-p', 'crm-stg.boxesos.com:%s:%s' % (exposed_port, exposed_port)])
...
class KVM(object):
    ...
    def start(self):
        kvm_cmd = [
            ...
            "-net", "user,hostfwd=tcp:crm-stg.boxesos.com:10022-:22,hostfwd=tcp:crm-stg.boxesos.com:18069-:8069,hostfwd=tcp:crm-stg.boxesos.com:15432-:5432",
            "-m", "2048",
    ...
    def ssh(self, cmd):
        run_cmd([
            ...
            '-p', '10022',
            '-i', self.ssh_key,
            '%s@crm-stg.boxesos.com' % self.login,
            cmd
        ]).check_returncode()
...
class KVMWinBuildExe(KVM):
    def run(self):
        ...
        self.rsync(['%s/' % self.args.build_dir, '%s@crm-stg.boxesos.com:%s' % (self.login, remote_build_dir)])
        self.ssh("cd {}setup/win32;time make allinone;".format(remote_build_dir))
        self.rsync(['%s@crm-stg.boxesos.com:%ssetup/win32/release/' % (self.login, remote_build_dir), '%s/' % self.args.build_dir])
...
class KVMWinTestExe(KVM):
    def run(self):
        ...
        self.rsync(['%s' % setup_path, '%s@crm-stg.boxesos.com:' % self.login])
